...
def draft_content_task(title: str, audience: str, platform: str, target_seconds: int) -> ContentTask:
    ...
    directives = [
        ...
        *(RenderDirective(id=f"fade-{beat.id}", type="transition.fade", beat_id=beat.id, fade_seconds=0.25) for beat in beats),
        ...
        RenderDirective(id="privacy-mask-example", type="privacy.mask", enabled=False, beat_id="decision", start_seconds=beats[3].start_seconds, end_seconds=min(beats[3].start_seconds + 5, beats[3].end_seconds), text="框选需遮挡的信息后启用", color="#101827", x=0.05, y=0.05, width=0.22, height=0.1, mask_mode="blur"),
    ]
