...
export interface VideoExport { file_name: string; subtitle_file_name: string; duration_seconds: number; size_bytes: number; }
export interface RenderDirective { id: string; type: 'card.show' | 'transition.fade' | 'focus.zoom' | 'highlight.rect' | 'audio.duck'; enabled: boolean; beat_id?: string | null; start_seconds: number; end_seconds: number; text?: string | null; color: string; x: number; y: number; width: number; height: number; fade_seconds: number; volume: number; }
...
export interface Task { task_id: string; title: string; audience: string; platform: Platform; target_seconds: number; status: 'prepare_assets' | 'ready_for_assembly' | 'rough_cut_ready'; voiceover_asset: Asset; music_asset?: Asset | null; beats: Beat[]; render_directives: RenderDirective[]; rough_cut?: { edit_plan: Record<string, unknown> } | null; video_export?: VideoExport | null; }
const base = '/api/v1';
