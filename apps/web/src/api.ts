...
export interface RenderDirective { id: string; type: 'card.show' | 'transition.fade' | 'focus.zoom' | 'highlight.rect'; enabled: boolean; beat_id?: string | null; start_seconds: number; end_seconds: number; text?: string | null; color: string; x: number; y: number; width: number; height: number; fade_seconds: number; }
...
export interface Task { task_id: string; title: string; audience: string; platform: Platform; target_seconds: number; status: 'prepare_assets' | 'ready_for_assembly' | 'rough_cut_ready'; voiceover_asset: Asset; beats: Beat[]; render_directives: RenderDirective[]; rough_cut?: { edit_plan: Record<string, unknown> } | null; video_export?: VideoExport | null; }
const base = '/api/v1';
