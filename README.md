...
第一版现已专注四个动作：创建内容任务、准备并上传素材、生成粗剪方案、自动导出基础 MP4。导出器会按分镜拼接画面、使用完整旁白作为主音轨，并叠加基础字幕；它仍不取代剪映或 Premiere，适合先生成可观看的初剪、再精修。
...
- 目前不处理转场、动效、智能镜头选取、敏感信息遮挡、BGM 混音或字幕逐词高亮；这些将在 [下一阶段计划](docs/NEXT_STAGE_PLAN.md) 中按可编辑、可追溯的渲染指令逐步实现。
